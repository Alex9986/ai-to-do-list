        ...
        <section className="w-full md:w-80 bg-slate-50 border-l border-slate-200 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((msg, i) => (
              <div
                key={i}
                className={`p-3 rounded-lg text-sm ${msg.role === "user" ? "bg-blue-50 text-blue-700 ml-4" : "bg-white border border-slate-200 text-slate-600 mr-4"}`}
              >
                {msg.content}
              </div>
            ))}
      ...
      <footer className="p-6 bg-white border-top border-slate-200 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
        <div className="max-w-3xl mx-auto relative">
          <div className="absolute left-4 top-1/2 -translate-y-1/2">
            <Sparkles
              className={`w-5 h-5 ${isProcessing ? "text-blue-500 animate-pulse" : "text-slate-300"}`}
            />
          </div>
          <input
            type="text"
